...
def immune_phenotyping(input_file, output_file, thresholds):
    ...
    for column in ['T_cells', 'CD4_Pos', 'CD4_Neg', 'B_cells', 'NK_cells', 'Monocytes_Macrophages', 'Granulocytes']:
        df[column] = False
    ...
    df.loc[mask, 'Monocytes_Macrophages'] = ~(df['CD3'] > thresholds['CD3']) & ~(df['CD20'] > thresholds['CD20']) & ~(df['CD56'] > thresholds['CD56']) & ((df['CD14'] > thresholds['CD14']) | (df['CD68'] > thresholds['CD68'])) & mask
    ...
    def classify_5_classes(row):
        if not row['CD45_Pos']:
            ...
            return 'NK-cells'
        elif row['Monocytes_Macrophages']:
            return 'Monocyte/Macrophages'
        else:
            return 'Granulocytes'
    ...
    print(f"Monocytes/Macrophages: {df['Monocytes_Macrophages'].sum()}")
    print(f"Granulocytes: {df['Granulocytes'].sum()}")
...
def main():
    ...
    # filter out all the CD45- cells because those are not immune cells and not helpful for model training
    df = pd.read_table(args.output_file, sep='\t')
    print(df.shape)
    df = df[df['Binary_classification'] != 'none']
